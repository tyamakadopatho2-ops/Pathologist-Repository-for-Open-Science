...
## NicheNet × Seurat v5: precompute all senders -> evaluate any sender set immediately-----------------
#  - Store expression (mdx / mdxD2) by condition (all senders)
#  - Calculate LFC after back-transforming to linear space with expm1 (mdxD2 / mdx)
#  - Integrate detectability (experimental tractability), sender specificity, and receptor support
#  - Allow immediate filtering and plotting for arbitrary sender sets

# NOTE:
#  - This NicheNet section was last validated in our local environment on 2026-04-22.
#  - Reproducible execution may depend on the installed nichenetr version, the
#  - R/Bioconductor environment, and the exact prior-model resource files used here
#  - (ligand-target matrix, ligand-receptor network, and weighted networks).
#  - Because NicheNet workflows and vignettes are updated over time, and some older
#  - workflows are no longer supported or code-fixed, users should verify package/resource
#  - compatibility and adapt this script as needed for their local system and current
#  - NicheNet release.
...
suppressPackageStartupMessages({
  ...
  library(patchwork);                      library(circlize)
  library(nichenetr);                      library(cli)
})
...
read_if <- function(p) { if (!file.exists(p)) stop("File not found: ", p, call. = FALSE); readRDS(p) }

muscle_sc            <- read_if(paths$seurat_obj)
ligand_target_matrix <- as.matrix(read_if(paths$ligand_target_matrix))
lr_network           <- read_if(paths$lr_network)
weighted_networks    <- read_if(paths$weighted_networks)

DefaultAssay(muscle_sc) <- "RNA"
if (sum(grepl("^counts", Layers(muscle_sc[["RNA"]]))) > 1) muscle_sc <- JoinLayers(muscle_sc)
if (!"data" %in% Layers(muscle_sc[["RNA"]])) {
  muscle_sc <- NormalizeData(muscle_sc, assay = "RNA",
                             normalization.method = "LogNormalize", verbose = FALSE)
}
...
condition_ref      <- "mdx"
condition_oi       <- "mdxD2"
conds_vec          <- c(condition_ref, condition_oi)

## Define all candidate senders (automatically exclude absent classes)
candidates_all <- c(
  "MHC-low macrophage","MHC-high macrophage","Endothelial cell(EC)","Stromal",
  "Immune-Stromal(Cd53+)","Tenocyte/Tendon fibroblast",
  "Neutrophil","Dendritic cell(DC)","Satellite cell","Pericyte/Vascular SMC","Schwann cell"
)
senders_all <- intersect(candidates_all, levels(Idents(muscle_sc)))
cli::cli_alert_info(paste("Senders found:", paste(senders_all, collapse = " | ")))

## 1) Run NicheNet once with all senders to capture a broad candidate pool ---------------
cli::cli_h1("Run NicheNet (senders = ALL)")
nichenet_output_all <- nichenet_seuratobj_aggregate(
  ...
  sender               = senders_all,              # ★ all senders
...
stopifnot(!is.null(nichenet_output_all$ligand_activities),
          nrow(nichenet_output_all$ligand_activities) > 0)

## Remove zero-row/zero-column ligands (compatibility safeguard)
ltm0_row <- rownames(ligand_target_matrix)[rowSums(ligand_target_matrix) == 0]
ltm0_col <- colnames(ligand_target_matrix)[colSums(ligand_target_matrix) == 0]
ltm0 <- unique(c(ltm0_row, ltm0_col))

la_all   <- nichenet_output_all$ligand_activities
lig_col  <- intersect(c("test_ligand","ligand","ligand_gene","ligand_symbol"), colnames(la_all))[1]
scorecol <- if ("aupr" %in% names(la_all)) "aupr" else "pearson"

mat_log <- tryCatch(GetAssayData(muscle_sc, assay = "RNA", layer = "data"),
                    error = function(e) GetAssayData(muscle_sc, assay = "RNA", slot = "data"))
mat_lin <- mat_log; mat_lin@x <- expm1(mat_lin@x)

## Construct an all-hit pool (to minimize dropouts): prioritize hit -> pearson > 0 -> all
if ("hit" %in% colnames(la_all)) {
  lig_pool <- unique(na.omit(as.character(la_all[[lig_col]][la_all$hit %in% c(TRUE,1)])))
} else if ("pearson" %in% colnames(la_all)) {
  lig_pool <- unique(na.omit(as.character(la_all[[lig_col]][la_all$pearson > 0])))
} else {
  lig_pool <- unique(na.omit(as.character(la_all[[lig_col]])))
}
lig_pool <- setdiff(intersect(lig_pool, rownames(mat_log)), ltm0)  # present in the data and not part of zero-only rows/columns
stopifnot(length(lig_pool) > 0)
cli::cli_alert_success(paste("Ligand pool size (ALL senders):", length(lig_pool)))

## 2) Precompute and cache expression/LFC for all sender × condition combinations ---------------
meta <- muscle_sc[[]]

## Tabulate expression frequency and linear-scale mean by sender × condition
expr_metrics_all <- lapply(senders_all, function(sdr){
  cells_all <- WhichCells(muscle_sc, idents = sdr)
  lapply(conds_vec, function(k){
    cells <- intersect(cells_all, rownames(meta)[meta[[condition_col]] == k])
    if (!length(cells)) return(NULL)
    sub_log <- mat_log[lig_pool, cells, drop = FALSE]
    sub_lin <- mat_lin[lig_pool, cells, drop = FALSE]
    data.frame(
      sender    = sdr,
      condition = k,
      ligand    = lig_pool,
      pct       = Matrix::rowSums(sub_log > 0) / ncol(sub_log) * 100,
      avg_lin   = as.numeric(Matrix::rowMeans(sub_lin)),
      stringsAsFactors = FALSE
    )
  }) |> bind_rows()
}) |> bind_rows()

## Tabulate linear-scale LFC (mdxD2 / mdx) for each sender
lfc_psc <- 0.1
lfc_all <- lapply(senders_all, function(sdr){
  all_cells <- WhichCells(muscle_sc, idents = sdr)
  oi_cells  <- intersect(all_cells, rownames(meta)[meta[[condition_col]] == condition_oi])
  ref_cells <- intersect(all_cells, rownames(meta)[meta[[condition_col]] == condition_ref])
  if (length(oi_cells) == 0 || length(ref_cells) == 0) return(NULL)
  m_oi  <- Matrix::rowMeans(mat_lin[lig_pool, oi_cells,  drop = FALSE])
  m_ref <- Matrix::rowMeans(mat_lin[lig_pool, ref_cells, drop = FALSE])
  data.frame(sender = sdr, ligand = lig_pool,
             lfc = log2((as.numeric(m_oi)+lfc_psc)/(as.numeric(m_ref)+lfc_psc)),
             stringsAsFactors = FALSE)
}) |> bind_rows()

## 3) Precompute receptor support (activation in Myocyte/Myonucleus) ------------------
summarize_receivers_from_lr <- function(
    ligands, receiver_id = "Myocyte/Myonucleus",
    cond_col = "genotype", assay = "RNA", layer = "data"
){
  stopifnot(cond_col %in% colnames(muscle_sc[[]]))
  DefaultAssay(muscle_sc) <- assay
  mlog <- tryCatch(GetAssayData(muscle_sc, layer = layer),
                   error = function(e) GetAssayData(muscle_sc, slot = layer))
  mlin <- mlog; mlin@x <- expm1(mlin@x)
  cells_recv <- WhichCells(muscle_sc, idents = receiver_id)
  meta <- muscle_sc[[]]; cond_vec <- meta[cells_recv, cond_col, drop = TRUE]
  ligands <- intersect(ligands, unique(lr_network$from))
  if (!length(ligands)) return(data.frame())
  out <- lapply(ligands, function(lg){
    recs <- intersect(unique(lr_network$to[lr_network$from == lg]), rownames(mlog))
    if (!length(recs)) return(NULL)
    do.call(rbind, lapply(recs, function(rc){
      xlog <- as.numeric(mlog[rc, cells_recv, drop = TRUE])
      xlin <- as.numeric(mlin[rc, cells_recv, drop = TRUE])
      pctv <- tapply(xlog > 0, cond_vec, mean) * 100
      avgv <- tapply(xlin,     cond_vec, mean)
      data.frame(ligand=lg, receptor=rc, condition=names(pctv),
                 pct=as.numeric(pctv), avg_lin=as.numeric(avgv))
    }))
  })
  bind_rows(out)
}

## Summarize receptor evidence for all candidates
rec_sum_all <- summarize_receivers_from_lr(lig_pool, receiver_id = receiver_oi, cond_col = condition_col)

## Receptor activation score (log2 ratio for mdxD2 vs mdx × mdxD2 expression frequency)
calc_receiver_support <- function(rec_sum, cond_ref = "mdx", cond_oi = "mdxD2", pseudo = 0.05){
  if (is.null(rec_sum) || !nrow(rec_sum)) return(data.frame(ligand=character(), receiver_support=numeric(), best_receptor=character()))
  rec_sum %>% group_by(ligand, receptor) %>%
    summarise(
      pct_ref = pct[condition==cond_ref], pct_oi = pct[condition==cond_oi],
      avg_ref = avg_lin[condition==cond_ref], avg_oi = avg_lin[condition==cond_oi],
      .groups="drop"
    ) %>%
    mutate(lfc_rec = log2((avg_oi+pseudo)/(avg_ref+pseudo)),
           pct_oi_n = pmin(pct_oi/100, 1),
           rec_contrib = pmax(lfc_rec, 0) * pct_oi_n) %>%
    group_by(ligand) %>%
    summarise(receiver_support = max(rec_contrib, na.rm = TRUE),
              best_receptor    = receptor[which.max(rec_contrib)], .groups="drop")
}
rec_support <- calc_receiver_support(rec_sum_all)

## (Reference) Also compute the proportion of receiver-side presence (legacy rec_frac)
get_genes <- function(idents, pct = 0.10){
  cells <- WhichCells(muscle_sc, idents = idents)
  m <- tryCatch(GetAssayData(muscle_sc, assay = "RNA", layer = "data"),
                error = function(e) GetAssayData(muscle_sc, assay = "RNA"))
  sub <- m[, cells, drop = FALSE]
  rownames(m)[ Matrix::rowSums(sub > 0) / ncol(sub) >= pct ]
}
genes_recv <- get_genes(receiver_oi, pct = 0.10)
rec_frac_by_lig <- setNames(
  sapply(lig_pool, function(lg){
    recs <- intersect(unique(lr_network$to[lr_network$from == lg]), rownames(muscle_sc[["RNA"]]))
    if (!length(recs)) 0 else mean(recs %in% genes_recv)
  }),
  lig_pool
)

## 4) General functions for selection modes and detectability ---------------------------------
thr_tbl <- list(
  lenient  = list(pearson_q = 0.20, min_pct = 0.05, min_avg_lin = 0.00, min_rec_support = 0.00, top_cap = 300),
  balanced = list(pearson_q = 0.40, min_pct = 0.10, min_avg_lin = 0.01, min_rec_support = 0.02, top_cap = 200),
  strict   = list(pearson_q = 0.70, min_pct = 0.15, min_avg_lin = 0.05, min_rec_support = 0.05, top_cap = 120)
)

## Generate a detectability table for an arbitrary sender set
make_detect_table <- function(senders_focus,
                              filter_mode = "lenient",
                              w_avg = 0.40, w_pct = 0.30, w_lfc = 0.20, w_rec = 0.10){
  thr <- thr_tbl[[filter_mode]]; stopifnot(!is.null(thr))
  sdr <- intersect(senders_focus, senders_all)
  if (!length(sdr)) stop("No sender classes were found.")
  la <- la_all
  
  ## Quantile threshold and top-cap for ligand_activities
  qs <- quantile(la[[scorecol]], probs = thr$pearson_q, na.rm = TRUE)
  la_keep <- la[[lig_col]][ la[[scorecol]] >= qs ]
  if ("pearson" %in% names(la)) la_keep <- intersect(la_keep, la[[lig_col]][ la$pearson > 0 ])
  la_ordered <- la[order(-la[[scorecol]]), , drop=FALSE]
  la_keep_top <- head(la_ordered[[lig_col]], thr$top_cap)
  lig_base <- unique(intersect(intersect(lig_pool, la_keep_top), la_keep))
  
  ## expr: average mdxD2 expression frequency and abundance (mean across senders)
  em_oi <- expr_metrics_all %>%
    filter(sender %in% sdr, condition == condition_oi, ligand %in% lig_base) %>%
    group_by(ligand) %>% summarise(pct = mean(pct), avg_lin = mean(avg_lin), .groups="drop")
  
  ## LFC: maximum absolute LFC within the selected senders
  lfc_max <- lfc_all %>%
    filter(sender %in% sdr, ligand %in% lig_base) %>%
    group_by(ligand) %>% summarise(abs_lfc = max(abs(lfc), na.rm = TRUE), .groups="drop")
  
  ## Sender specificity (max / sum across senders in mdxD2)
  ssi_tbl <- expr_metrics_all %>%
    filter(sender %in% sdr, condition == condition_oi, ligand %in% lig_base) %>%
    group_by(ligand, sender) %>% summarise(avg_lin = mean(avg_lin), .groups="drop") %>%
    group_by(ligand) %>%
    summarise(ssi = max(avg_lin)/(sum(avg_lin)+1e-9),
              top_sender = sender[which.max(avg_lin)], .groups="drop")
  
  ## Merge receptor-support scores
  rec_sup_sub <- rec_support %>% filter(ligand %in% lig_base)
  if (!nrow(rec_sup_sub)) rec_sup_sub <- data.frame(ligand=lig_base, receiver_support=0, best_receptor=NA)
  
  ## Filter by expression frequency, abundance, and receptor support
  cand <- em_oi %>%
    left_join(lfc_max, by="ligand") %>%
    left_join(ssi_tbl, by="ligand") %>%
    left_join(rec_sup_sub, by="ligand") %>%
    mutate(pass = (pct >= thr$min_pct*100) & (avg_lin >= thr$min_avg_lin) &
             (receiver_support >= thr$min_rec_support)) %>%
    filter(pass)
  
  ## Detectability (rescale to 0–1 and apply weights)
  rng01 <- function(x){ if (all(!is.finite(x)) || diff(range(x, na.rm=TRUE))==0) return(rep(0,length(x)))
    scales::rescale(x, to=c(0,1), from=range(x, na.rm=TRUE)) }
  out <- cand %>%
    mutate(
      pct_n = pmin(pct/100, 1),
      avg_n = rng01(avg_lin),
      lfc_n = rng01(abs_lfc),
      rec_n = rng01(receiver_support),
      detect = w_avg*avg_n + w_pct*pct_n + w_lfc*lfc_n + w_rec*rec_n,
      detect2 = detect + 0.10*pmax(0, ssi - 0.5),  # apply a small bonus for sender specificity
      class  = case_when(
        pct >= 30 & avg_lin >= 2.0 ~ "IHC standard",
        pct >= 15 & avg_lin >= 0.5 ~ "IHC (TSA amplification)",
        TRUE                       ~ "RNAscope recommended"
      )
    ) %>%
    arrange(desc(detect2))
  out
}
...
## Example A: Pericyte/Vascular + Satellite
det_A <- make_detect_table(c("Pericyte/Vascular SMC","Satellite cell"), filter_mode = "balanced")
print(det_A %>% select(ligand, detect2, class, top_sender, ssi) %>% head(10))
#   ligand  detect2 class          top_sender            ssi
# 1 Col18a1   0.802 IHC standard   Pericyte/Vascular SMC 0.809
# 2 Timp1     0.704 IHC (TSA amplification) Satellite cell        0.733
# 3 Cd47      0.571 IHC standard   Pericyte/Vascular SMC 0.552
# 4 Angptl4   0.547 IHC (TSA amplification) Satellite cell        0.641
# 5 Lamb2     0.443 IHC (TSA amplification) Pericyte/Vascular SMC 0.877
# 6 Dusp18    0.102 RNAscope recommended  Satellite cell        0.629

## Example B: Endothelial + Stromal
det_B <- make_detect_table(c("Endothelial cell(EC)","Stromal"), filter_mode = "balanced")
print(det_B %>% select(ligand, detect2, class, top_sender, ssi) %>% head(10))
#   ligand  detect2 class          top_sender            ssi
# 1 Timp1     0.873 IHC standard   Stromal              0.917
# 2 Cd47      0.530 IHC standard   Stromal              0.525
# 3 Angptl4   0.477 IHC (TSA amplification) Stromal              0.793
# 4 Lamb2     0.297 IHC (TSA amplification) Stromal              0.638
# 5 Col18a1   0.182 IHC (TSA amplification) Endothelial cell(EC) 0.653

## Example C: check Tenocyte/Tendon fibroblast + MHC-low macrophage
det_C <- make_detect_table(c("Tenocyte/Tendon fibroblast","MHC-low macrophage"), filter_mode = "balanced")
print(det_C %>% select(ligand, detect2, class, top_sender, ssi) %>% head(10))
#   ligand  detect2 class          top_sender                 ssi
# 1 Timp1    0.863  IHC standard   Tenocyte/Tendon fibroblast 0.948
# 2 Cd47     0.535  IHC standard   MHC-low macrophage         0.601
# 3 Angptl4  0.488  IHC (TSA amplification) Tenocyte/Tendon fibroblast 0.952
# 4 Lamb2    0.292  IHC (TSA amplification) Tenocyte/Tendon fibroblast 0.973
# 5 Col18a1  0.209  IHC (TSA amplification) Tenocyte/Tendon fibroblast 0.971
# 6 Dusp18   0.0835 RNAscope recommended  Tenocyte/Tendon fibroblast 0.586

## Example D: check all selected senders
det_D <- make_detect_table(c("Pericyte/Vascular SMC","Stromal",
                             "Tenocyte/Tendon fibroblast","Satellite cell",
                             "Endothelial cell(EC)","MHC-high macrophage",
                             "MHC-low macrophage"), filter_mode = "strict")
print(det_D %>% select(ligand, detect2, class, top_sender, ssi) %>% head(10))
#   ligand  detect2 class          top_sender                 ssi
# 1 Timp1     0.836 IHC standard   Tenocyte/Tendon fibroblast 0.422
# 2 Cd47      0.618 IHC standard   MHC-high macrophage        0.180
# 3 Col18a1   0.296 IHC (TSA amplification) Pericyte/Vascular SMC      0.541
# 4 Lamb2     0.186 IHC (TSA amplification) Pericyte/Vascular SMC      0.365


## ======================== Utilities for arbitrary sender sets =========================

## 1) Quickly suggest top ligands for an arbitrary sender set (ranked by detectability)
suggest_ligands_for_senders <- function(senders_focus, top_n = 50, filter_mode = "lenient"){
  det <- make_detect_table(senders_focus, filter_mode = filter_mode)
  head(det$ligand, min(top_n, nrow(det)))
}

## 2) Draw expression × LFC from cached results (enhanced with automatic ligand selection)
plot_expr_lfc_from_cache <- function(
    senders_focus,
    ligands_to_plot = NULL,     # If NULL, automatically select the top_n ligands
    top_n = 50,
    dot_max_size = 6,
    lfc_limits = NULL
){
  sdr <- intersect(senders_focus, senders_all)
  if (!length(sdr)) stop("No sender classes were found.: ", paste(senders_focus, collapse=", "))
  
  lig <- if (is.null(ligands_to_plot)) suggest_ligands_for_senders(sdr, top_n = top_n) else ligands_to_plot
  lig <- intersect(lig, lig_pool)
  stopifnot(length(lig) > 0)
  
  expr_df <- expr_metrics_all %>% dplyr::filter(sender %in% sdr, ligand %in% lig)
  lfc_df  <- lfc_all          %>% dplyr::filter(sender %in% sdr, ligand %in% lig)
  
  expr_df$ligand <- factor(expr_df$ligand, levels = rev(lig))
  lfc_df$ligand  <- factor(lfc_df$ligand,  levels = rev(lig))
  expr_df$sender <- factor(expr_df$sender, levels = sdr)
  lfc_df$sender  <- factor(lfc_df$sender,  levels = sdr)
  
  if (is.null(lfc_limits)) {
    lim <- stats::quantile(lfc_df$lfc, probs = c(0.05, 0.95), na.rm = TRUE)
    m   <- max(abs(lim)); lfc_limits <- c(-m, m)
  }
  
  p_expr <- ggplot(expr_df, aes(x = ligand, y = sender)) +
    geom_point(aes(size = pct, fill = avg_lin),
    ...
    scale_fill_distiller(palette = "RdYlBu", direction = -1,
                         name = "Avg expression (linear TP10k)") +
    labs(title = "Expression in Sender (by condition)", x = NULL, y = NULL) +
  ...
  p_lfc <- ggplot(lfc_df, aes(x = ligand, y = sender, fill = lfc)) +
    geom_tile(colour = "white", linewidth = 0.25) +
    scale_fill_gradient2(low = "#2166AC", high = "#B2182B", mid = "white",
                         limits = lfc_limits, oob = scales::squish,
                         name = paste0("LFC (", condition_oi, " / ", condition_ref, ")")) +
    labs(title = "LFC in Sender (linear)", x = NULL, y = NULL) +
    theme_bw(base_size = 11) +
    theme(panel.grid = element_blank(),
          axis.text.x = element_text(angle = 60, hjust = 1, vjust = 1)) +
    coord_flip()
  
  p_comb <- p_expr + p_lfc + patchwork::plot_layout(ncol = 2, widths = c(1.2, 1))
  list(expr_df = expr_df, lfc_df = lfc_df, expr_plot = p_expr, lfc_plot = p_lfc, combined = p_comb)
}

## 3) Chord diagram for an arbitrary sender set (composite score)
plot_chord_for_senders <- function(
    senders_focus,
    ligands_to_plot = NULL,
    top_n_lig = 50,
    N_per_lig = 150,
    N_global  = 250,
    w_rp  = 0.45, w_act = 0.25, w_rec = 0.15, w_lfc = 0.15,
    pct_expr_sender = 0.10,                   # Expression-frequency threshold used to consider the sender as producing the ligand
    expr_condition   = c("max","mdxD2","mdx"),
    color_mode       = c("top","origin4","origin_any"),
    ## --- Labeling parameters ---
    label_n_lig = 200,    # Maximum number of ligand labels to display (top by summed score)
    label_n_tgt = 80,     # Maximum number of target labels to display (top by summed score)
    label_cex    = 0.70,  # text size
    label_offset_mm = 1.2 # outward label offset (mm)
){
  expr_condition <- match.arg(expr_condition)
  ...
  ## Ligand set
  ligs <- if (is.null(ligands_to_plot)) suggest_ligands_for_senders(sdr, top_n = top_n_lig) else ligands_to_plot
  ligs <- intersect(ligs, lig_pool)
  stopifnot(length(ligs) > 0)
  
  ## Orient the ligand-target matrix as ligand rows and target columns
  mat <- ligand_target_matrix
  if (sum(ligs %in% rownames(mat)) < sum(ligs %in% colnames(mat))) mat <- t(mat)
  
  ## Target candidates
  tgt_key <- intersect(c("top_targets","prioritized_targets"), names(nichenet_output_all))[1]
  targets_all <- if (length(tgt_key)) unique(unlist(nichenet_output_all[[tgt_key]])) else colnames(mat)
  lig_ok <- intersect(ligs, rownames(mat))
  tgt_ok <- intersect(targets_all, colnames(mat))
  stopifnot(length(lig_ok) > 0, length(tgt_ok) > 0)
  
  ## Top targets for each ligand
  ...
  ## Sender-of-origin (sender with the highest linear mean in mdxD2)
  ssi_tbl <- expr_metrics_all %>%
    dplyr::filter(sender %in% sdr, condition == condition_oi, ligand %in% lig_ok) %>%
    dplyr::group_by(ligand, sender) %>% dplyr::summarise(avg_lin = mean(avg_lin), .groups="drop") %>%
    dplyr::group_by(ligand) %>%
    dplyr::summarise(top_sender = sender[which.max(avg_lin)],
                     ssi = max(avg_lin)/(sum(avg_lin)+1e-9), .groups="drop")
  
  ## LFC (maximum absolute LFC across selected senders)
  lfc_by_lig <- lfc_all %>%
    dplyr::filter(sender %in% sdr, ligand %in% lig_ok) %>%
    dplyr::group_by(ligand) %>%
    dplyr::summarise(abs_lfc = max(abs(lfc), na.rm = TRUE), .groups="drop") %>%
    { setNames(.$abs_lfc, .$ligand) }
  
  ## ligand_activities (from the all-sender run)
  act_vec <- setNames(la_all[[scorecol]], la_all[[lig_col]])
  
  ## Receptor activation score (Myocyte/Myonucleus)
  rec_sup_sub <- rec_support %>%
    dplyr::filter(ligand %in% lig_ok) %>%
    { if (!nrow(.)) data.frame(ligand=lig_ok, receiver_support=0, best_receptor=NA) else . }
  ...
  pct_rank <- function(x){ rx <- rank(x, ties.method = "average", na.last = "keep"); (rx - 1) / (sum(!is.na(x)) - 1) }
  
  lig_act_rnk <- data.frame(ligand = lig_ok, act_rnk = pct_rank(act_vec[lig_ok]))
  lig_rec_sup <- rec_sup_sub %>% dplyr::mutate(rec_n = rescale01(receiver_support)) %>% dplyr::select(ligand, rec_n)
  lig_lfc_rnk <- data.frame(ligand = lig_ok, lfc_rnk = pct_rank(lfc_by_lig[lig_ok]))
  
  plot_df <- links_df %>%
    dplyr::mutate(rp_rnk = pct_rank(regulatory_potential)) %>%
    dplyr::left_join(lig_act_rnk, by = "ligand") %>%
    dplyr::left_join(lig_rec_sup, by = "ligand") %>%
    dplyr::left_join(lig_lfc_rnk, by = "ligand") %>%
    dplyr::left_join(ssi_tbl,     by = "ligand") %>%
    dplyr::mutate(
      act_rnk = ifelse(is.finite(act_rnk), act_rnk, 0),
      rec_n   = ifelse(is.finite(rec_n),   rec_n,   0),
      lfc_rnk = ifelse(is.finite(lfc_rnk), lfc_rnk, 0),
      score   = w_rp*rp_rnk + w_act*act_rnk + w_rec*rec_n + w_lfc*lfc_rnk,
      ligandL = paste0("L:", ligand),
      targetT = paste0("T:", target)
    ) %>%
    dplyr::arrange(dplyr::desc(score)) %>%
    dplyr::slice_head(n = N_global)
  if (!nrow(plot_df)) stop("Chord: no links are available for visualization. Increase N_global or N_per_lig.")
  
  ## ========== Coloring mode (choose top / origin4 / origin_any) ==========
  expr_tbl <- expr_metrics_all %>%
    dplyr::filter(sender %in% sdr, ligand %in% lig_ok) %>%
    { switch(expr_condition,
             "mdxD2" = dplyr::filter(., condition == condition_oi),
             "mdx"   = dplyr::filter(., condition == condition_ref),
             "max"   = dplyr::group_by(., ligand, sender) %>% dplyr::summarise(pct = max(pct), .groups="drop")) } %>%
    dplyr::mutate(expressed = pct >= 100*pct_expr_sender) %>%
    dplyr::group_by(ligand) %>%
    dplyr::summarise(
      n_exp = sum(expressed),
      one_sender = if (sum(expressed)==1) sender[which(expressed)][1] else NA_character_,
      .groups = "drop"
    )
  
  if (color_mode == "origin4" && length(sdr) == 2) {
    group_df <- expr_tbl %>% dplyr::mutate(group = dplyr::case_when(
      n_exp == 0 ~ "Others",
      n_exp >= 2 ~ "Both",
      n_exp == 1 ~ one_sender
    ))
    col_map <- c(
      setNames(c("#2874c5", "#45a045"), sdr),  # sender1 = blue, sender2 = green
      Both   = "#7fff00",
      Others = "#999999"
    )
    legend_items <- c(sdr, "Both", "Others")
  } else if (color_mode == "origin_any") {
    group_df <- expr_tbl %>% dplyr::mutate(group = dplyr::case_when(
      n_exp == 0 ~ "Others",
      n_exp >= 2 ~ "Multi",
      n_exp == 1 ~ one_sender
    ))
    pal_hcl <- function(n){ grDevices::hcl(h = seq(15, 375, length.out = n + 1)[- (n + 1)], c = 100, l = 65) }
    sender_cols <- setNames(pal_hcl(length(sdr)), sdr)
    col_map <- c(sender_cols, Multi = "#7fff00", Others = "#999999")
    legend_items <- c(names(sender_cols), "Multi", "Others")
  } else {
    group_df <- ssi_tbl %>% dplyr::transmute(ligand, group = top_sender)
    pal_hcl <- function(n){ grDevices::hcl(h = seq(15, 375, length.out = n + 1)[- (n + 1)], c = 100, l = 65) }
    col_map <- setNames(pal_hcl(length(sdr)), sdr)
    legend_items <- names(col_map)
  }
  
  plot_df <- plot_df %>% dplyr::left_join(group_df, by = "ligand")
  link_cols <- col_map[ match(plot_df$group, names(col_map)) ]
  link_cols[is.na(link_cols)] <- "#999999"
  edge_col <- grDevices::adjustcolor(link_cols, alpha.f = 0.40)
  ## ===============================================================
  
  ## Sector colors (sender = blue / receiver = red)
  lig_sectors <- unique(plot_df$ligandL)
  tgt_sectors <- unique(plot_df$targetT)
  sectors <- c(lig_sectors, tgt_sectors)
  grid_col <- c(
    setNames(rep("#2874c5", length(lig_sectors)), lig_sectors),
    setNames(rep("#d73027", length(tgt_sectors)), tgt_sectors)
  )
  link_lwd <- scales::rescale(plot_df$score, to = c(1.2, 6.0))
  
  ## ---- Draw on the next page of the current graphics device ----
  grid::grid.newpage()
  op <- par(no.readonly = TRUE); on.exit({ par(op); circlize::circos.clear() }, add = TRUE)
  ...
  if (!base::setequal(sectors, cats_x)) {
  ...
  ## --- ★ Outer labels (previously omitted) ---
  keep_lig <- plot_df %>% dplyr::group_by(ligand) %>% dplyr::summarise(s = sum(score), .groups="drop") %>%
    dplyr::arrange(dplyr::desc(s)) %>% dplyr::slice_head(n = label_n_lig) %>% dplyr::pull(ligand)
  keep_tgt <- plot_df %>% dplyr::group_by(target) %>% dplyr::summarise(s = sum(score), .groups="drop") %>%
    dplyr::arrange(dplyr::desc(s)) %>% dplyr::slice_head(n = label_n_tgt) %>% dplyr::pull(target)
  ...
  circlize::circos.trackPlotRegion(
    ...
    panel.fun = function(x,y){
      ...
      y0  <- circlize::CELL_META$ylim[1] - circlize::mm_y(label_offset_mm)
      if (startsWith(nm,"L:") && nm %in% keep_ligL)
        circlize::circos.text(x0, y0, lab, facing="clockwise", niceFacing=TRUE,
                              adj=c(0,0.5), cex=label_cex, col="#2874c5")
      if (startsWith(nm,"T:") && nm %in% keep_tgtT)
        circlize::circos.text(x0, y0, lab, facing="clockwise", niceFacing=TRUE,
                              adj=c(0,0.5), cex=label_cex, col="#d73027")
    }
  )
  
  ## --- Right side: legends and title ---
  par(mar = c(2,1.5,1,2)); plot.new(); par(usr = c(0,1,0,1))
  legend(0.05,0.95, legend = names(col_map),
         fill = unname(col_map), border = NA, bty = "n", cex = 0.95, title = "Link color (origin)")
  legend(0.05,0.72, legend = c("Sender sectors","Receiver sectors"),
         fill = c("#2874c5","#d73027"), border = NA, bty = "n", cex = 0.95, title = "Sector color")
  text(0.05, 0.56, "Composite score", adj = 0, font = 2, cex = 1.05)
  ...
  mtext(paste0(condition_oi, " vs ", condition_ref, ": ",
               paste(sdr, collapse=" / "),
               " \u2192 ", receiver_oi),
        outer = TRUE, side = 3, line = 1.4, font = 2, cex = 1.25)
  
  invisible(list(plot_df = plot_df))
}
...
# Stromal&EC, Pericyte&Tendon, Satellite&Macro-low
## Use Both/Others for Pericyte & Satellite, with more labels and slightly larger text
plot_chord_for_senders(
  c("Stromal","Endothelial cell(EC)"),
  color_mode = "origin4",
  label_n_lig = 40,     # up to 40 ligand labels
  label_n_tgt = 80,     # up to 80 target labels
  label_cex = 1,      # slightly larger text size
  label_offset_mm = 0.1, # add more outward spacing
  pct_expr_sender = 0.5
)

plot_chord_for_senders(
  c("Pericyte/Vascular SMC","Tenocyte/Tendon fibroblast"),
  color_mode = "origin4",
  label_n_lig = 40,     # up to 40 ligand labels
  label_n_tgt = 80,     # up to 80 target labels
  label_cex = 1,      # slightly larger text size
...
plot_chord_for_senders(
  c("Satellite cell","MHC-low macrophage"),
  color_mode = "origin4",
  label_n_lig = 40,     # up to 40 ligand labels
  label_n_tgt = 80,     # up to 80 target labels
  label_cex = 1,      # slightly larger text size
...
plot_chord_for_senders(
  c("Schwann cell","MHC-high macrophage"),
  color_mode = "origin4",
  label_n_lig = 40,     # up to 40 ligand labels
  label_n_tgt = 80,     # up to 80 target labels
  label_cex = 1,      # slightly larger text size
